...
interface User {
  id: number;
  backendId?: string; // MongoDB ObjectId for backend users
  userId: string;
  name: string;
  age: number;
  distance: string;
...
export function HomePage() {
  console.log('🏠🏠🏠 HomePage component DEFINITELY loaded!');
  ...
  const [users, setUsers] = useState<User[]>([]); // Start empty
  const [isLoadingUsers, setIsLoadingUsers] = useState(false);

  const carouselRef = useRef<HTMLDivElement>(null);

  console.log('🏠 HomePage state - users:', users.length, 'isLoadingUsers:', isLoadingUsers);
  ...
  const fetchNearbyUsers = async () => {
    try {
      ...
      if (response.ok && data.status && data.data.users) {
        console.log('📊 API Response data:', data);
        console.log('📊 Users in response:', data.data?.users?.length || 0);
        ...
        const backendUsers = data.data.users.map((user: any, index: number) => ({
          ...
          bio: user.bio || 'LocalityBay user exploring the community!',
          profession: 'Professional',
          location: user.location?.city || 'Delhi',
        ...
        // Combine backend users with mock users
        const allUsers = [...mockUsers, ...backendUsers];
        console.log('🎯 Total users to display:', allUsers.length);
        setUsers(allUsers);
      } else {
        console.log('❌ API failed or no users, using mock data');
        setUsers(mockUsers);
      }
  ...
  const fetchActiveAreas = async () => {
    try {
      ...
      if (response.ok && data.status && data.data.activeAreas) {
        console.log('📊 Active areas response:', data.data.activeAreas);
        setActiveAreas(data.data.activeAreas);
        
        // Set the most active area (first one with highest online users)
        if (data.data.activeAreas.length > 0) {
          const topArea = data.data.activeAreas[0];
          setMostActiveArea({
            name: topArea.area || topArea.city,
            userCount: topArea.onlineUsers + topArea.totalUsers,
            location: `${topArea.area}, ${topArea.city}`,
            coordinates: topArea.centerCoordinates
          });
        }
      }
      setIsLoadingActiveAreas(false);
    } catch (error) {
  ...
  React.useEffect(() => {
    console.log('🚀 useEffect triggered - fetching users and active areas...');
    console.log('📍 Location dependency:', location);
    // Set mock users first, then fetch real users
    setUsers(mockUsers);
    fetchNearbyUsers();
    fetchActiveAreas();
  }, []);
  ...
  // State for active areas
  const [activeAreas, setActiveAreas] = useState([]);
  const [isLoadingActiveAreas, setIsLoadingActiveAreas] = useState(false);
  const [showActiveAreasModal, setShowActiveAreasModal] = useState(false);
  const [mostActiveArea, setMostActiveArea] = useState({
  ...
  console.log('🗺️ Users being passed to SimpleMap:', filteredUsers.map(u => ({ id: u.id, name: u.name })));
  ...
  const handleViewProfile = (user: User) => {
    ...
    setIsUserCardOpen(false);
    // Use backendId if available, otherwise use the frontend id for mock users
    const profileId = user.backendId || user.id;
  ...
  const handlePostMessage = async () => {
    ...
    try {
      ...
      if (response.ok && data.status) {
        console.log('✅ Message posted successfully:', data.data.message);
        setMessage("");
        // Show success feedback
        alert('Message posted successfully! It will appear on your profile on the map.');
      } else {
        console.error('❌ Failed to post message:', data.message);
        alert(data.message || 'Failed to post message. Please try again.');
      }
    } catch (error) {
  ...
  const handleSelectActiveArea = async (area: any) => {
    try {
      ...
      // Clear any existing filters to show only area users
      setInterestSearch("");
      ...
      if (response.ok && data.status && data.data.users) {
        // Convert backend users to frontend format
        const areaUsers = [...data.data.users.online, ...data.data.users.offline].map((user, index) => ({
          id: index + 300, // Avoid ID conflicts
          backendId: user._id, // Store the actual backend ID
          userId: user.userId || `@${user.name.toLowerCase().replace(' ', '')}`,
          name: user.name,
          age: user.dateOfBirth ? new Date().getFullYear() - new Date(user.dateOfBirth).getFullYear() : 25,
  ...
  return (
    ...
    >
          ...
          <CardContent className="p-3 sm:p-4 space-y-2 sm:space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              ...
              <Button
                ...
                disabled={isLoadingActiveAreas || isLoadingUsers}
                className="p-2 bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 hover:from-green-100 hover:to-blue-100 hover:border-green-300 transition-all duration-300 rounded-lg disabled:opacity-50"
              >
                <div className="flex items-center space-x-1">
                  {isLoadingUsers ? (
                  ...
                  <span className="text-xs font-semibold text-gray-700">
                    {isLoadingUsers ? 'Loading...' : 'Active Area'}
                  </span>
                </div>
              </Button>
            </div>
            ...
            <div className="p-2 bg-green-50 rounded-lg border border-green-200 text-xs text-gray-600">
              <div className="flex items-center space-x-2">
                ...
                {isLoadingActiveAreas ? (
                  <span className="animate-pulse">Loading...</span>
                ) : (
                  <span>{mostActiveArea.name} | {mostActiveArea.userCount} users</span>
                )}
              </div>
              <p className="mt-1 text-xs">
                Click "Active Area" to navigate to the busiest location and see users there!
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              <div
                ...
                onClick={() => {
                  ...
                  if (community.active) {
                    ...
                    navigate(
                      `${ROUTES.COMMUNITIES}?community=${encodeURIComponent(
                        community.name
                      )}`
                    );
                  } else {
                    ...
                    navigate(
                      `${ROUTES.COMMUNITIES}?location=${encodeURIComponent(
                        location
                      )}`
                    );
                  }
      ...
      {/* Active Areas Modal */}
      <Sheet open={showActiveAreasModal} onOpenChange={setShowActiveAreasModal}>
        <SheetContent side="bottom" className="h-[70vh] rounded-t-3xl border-none">
          <div className="card-enhanced card-cream h-full flex flex-col p-4">
            <SheetHeader className="pb-4">
              ...
              <SheetDescription>
                Select an area to view users and update your map location
              </SheetDescription>
            </SheetHeader>
              ...
              {isLoadingActiveAreas ? (
                ...
                activeAreas.map((area, index) => (
                  <div
                    ...
                    onClick={() => handleSelectActiveArea(area)}
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Navigation className="h-4 w-4 text-blue-600 group-hover:text-blue-700" />
                    </div>
                  </div>
                ))
              ) : (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600">No active areas found</p>
                </div>
              )}
            </div>
          </div>
